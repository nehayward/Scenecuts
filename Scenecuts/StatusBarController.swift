...
class StatusBarController {
    ...
    func createStatusItem(from scene: SceneStatusBarItem) -> NSStatusItem {
        ...
        statusItem.button?.identifier = NSUserInterfaceItemIdentifier(scene.name)
    ...
    @objc func triggerAction(_ sender: NSStatusBarButton) {
        guard let name = sender.identifier?.rawValue,
              let id = HelperManager.shared.helper.scenes.first(where: { $0.name == name})?.id else { return }
    ...
    func addMenuItem(for actionSet: ActionSet) {
        ...
        menuItem.identifier = NSUserInterfaceItemIdentifier(actionSet.name)
        menuItem.target = self
        menuItem.setShortcut(for: KeyboardShortcuts.Name(rawValue: actionSet.name))
        
        var iconName: String = SceneStatusBarItem.value(name: actionSet.name, forKey: .iconName) ?? ""
        if iconName.isEmpty {
        ...
        if menu.items.contains(where: { (item) -> Bool in
            item.identifier?.rawValue == actionSet.name
        }) {
            return
        }
    ...
    func removeMenuItem(with name: String) {
        guard let menu = mainStatusItem.menu else { return }
        
        menu.items.removeAll { (menuItem) -> Bool in
            menuItem.identifier?.rawValue == name
        }
    }
    ...
    func clearHomeKitScenes() {
        ...
        HelperManager.shared.helper.scenes.forEach { (scene) in
            menu.items.removeAll { (menu) -> Bool in
                menu.identifier?.rawValue == scene.name
            }
        }
    }
